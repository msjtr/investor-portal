...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const triggerError = (el) => {
        ...
        if (group) { group.classList.add('input-error'); setTimeout(() => group.classList.remove('input-error'), 3000); }
    };
    ...
    if (forgotForm) {
        forgotForm.addEventListener('submit', async (e) => {
            ...
            if (!validateEmail(email)) {
                showNotification("يرجى إدخال بريد إلكتروني صحيح ومسجل.", "warning"); triggerError(emailInput); return;
            }
            ...
            try {
                ...
                logSecurityEvent(email, "system_error", "failed", securityData, `خطأ فني: ${error.message}`);
